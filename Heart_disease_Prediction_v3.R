...
#heart dataset contains factors
#heart2 dataset have all the variables as numerical
...
heart2 <- read.csv('hearth.csv')
str(heart2) #numeric variables
...
sum(is.na(heart)) #There are not missing values in the dataset
...
cp1<- ggplot(heart, aes(cp, fill=target)) + geom_bar(stat="count", position= "fill") + labs(x="Chest Pain Type", fill= "Target", y="Stacked Count")
cp1
#We can see that the cp type 1 = Typical angina has the biggest incidence of heart disease compared to the others. However, the graph indicates that cp type 2 and 3 have an important relationship to heart disease.

#Target and Thalach (maximum heart rate achieved)
t1<- ggplot(heart, aes(thalach, col=target, fill=target))+ geom_density(alpha=0.2)+ guides(col=F)+ labs(fill="Target", x="Maximum heart rate achieved")
t2<- ggplot(heart, aes(target, thalach, fill=target))+ geom_boxplot() + labs(y="Maximum heart rate achieved", x="Target", fill="Target")

#We can see that the heart disease count is present when the thalach is over 150. The median of this variable is 153
...
#The stress level of 2 indicates the mayor incidence to have heart disease.
...
ca1
#0 and 3 vessels indicate a mayor incidence to have heart disease

#Target and oldpeak
op1<- ggplot(heart, aes(oldpeak, col=target, fill=target))+ geom_density(alpha=0.2)+ guides(col=F)+ labs(fill="Target", x="oldpeak")
...
#Target and exang
exa1<- ggplot(heart, aes(exang, fill=target)) + geom_bar(stat="count", position= "fill") + labs(x="Exercise Indiced angina", fill= "Target", y="Stacked Count")
#The level 0=no exercise angina indicase a count with patients with heart disease compared to level 1 = yes

#Target and sex
sex1<- ggplot(heart, aes(sex, fill=target)) + geom_bar(stat="count", position= "fill") + labs(x="Genre", fill= "Target", y="Stacked Count")
...
grid.arrange(cp1, t1, t2, thal1, ca1, op1, op2, slope1, rest1, exa1, sex1, excp, o, nrow=5)


#LOGISTIC REGRESSION MODEL  >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

#Original Logistic Regression Model
heart_mod1 <- glm(target ~., family = "binomial", data = heart)
summary(heart_mod1) #The AIC is 225.63

heart_mod_red <- step(heart_mod1, direction = "backward")
...
heart_mod2 <- glm(target ~., family = "binomial", data = new_heart)
summary(heart_mod2)
...
anova(heart_mod1, heart_mod2, test="Chisq") #Fail in reject the null hypothesis. The reduced model is better than the original model
...
set.seed(142)
train_data <- createDataPartition(new_heart$target, p=0.8, list=FALSE)
...
set.seed(25)
train_data2 <- createDataPartition(heart$target, p=0.8, list=FALSE)
